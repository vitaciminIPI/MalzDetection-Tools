...
def run(pluginName, filePath, outputPath, argument):
    ...
    # getting argument
    for opt in argument.keys():
        if opt == "pid":
            args.pid = [argument[opt]]
        elif opt == "dump":
            args.dump = argument[opt]
        elif opt == "physical":
            args.physical = argument[opt]
        elif opt == "key":
            args.key = argument[opt]
        elif opt == "offset":
            args.offset = argument[opt]
        elif opt == "include_corrupt":
            args.inculde_corrupt = argument[opt]
        else:
            continue
